...
def run(
    ...
    ## Load dataset ##
    ##################
    dataset = ClassificationDataset(f'{directory_dataset}/{dataset_name}_train.csv',
                                    input_dims=input_dims, output_dim=output_dim,
                                    transform_min=transform_min, transform_max=transform_max,
                                    seed=seed_dataset) 
    train_loader, val_loader, N = dataset.split_train_vad(batch_size=batch_size, num_workers=4)
    
    dataset = ClassificationDataset(f'{directory_dataset}/{dataset_name}_test.csv',
                                    input_dims=input_dims, output_dim=output_dim,
                                    transform_min=transform_min, transform_max=transform_max,
                                    seed=seed_dataset) 
    test_loader = torch.utils.data.DataLoader(dataset, batch_size=2 * batch_size, num_workers=4, pin_memory=True)
